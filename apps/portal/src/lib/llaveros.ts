...
export type ResultadoPreparacionLlavero = {
  id: string
  vecino_id: string
  solicitud_id: string
...
export async function programarEntregaLlavero(
  solicitudId: string,
...
export async function prepararLlavero(
  solicitudId: string,
  token: string,
...
export async function registrarEntregaLlavero(solicitudId: string) {
  const { data, error } = await supabase.rpc('registrar_entrega_llavero', {
    p_solicitud_id: solicitudId,
  })
...
export async function cambiarEstadoLlavero(
  llaveroId: string,
